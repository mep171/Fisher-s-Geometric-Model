# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 14:10:31 2026
...
def main():
    ...
    # --- Data Loading ---
    try:
        MiceG12C = pd.read_csv("Figure5A.csv")
        MiceG12D = pd.read_csv("Figure5B.csv")
        MiceEGFR = pd.read_csv("Figure5D.csv")
    ...
    for i in range(NUM_SEEDS):
        ...
        try:
            ...
            weights = 1.0 / jnp.square(norm)
            r2 = calculate_r_squared(observed, pred_fit)
            wr2 = calculate_r_squared(observed, pred_fit, weights=weights)

            Z_runs.append(np.array(fZ))
            P_runs.append(np.array(fP))
            results_list.append({"seed": i, "loss": float(res.state.fun_val), "r2": float(r2), "wr2": float(wr2)})
    ...
    LOSS_WINDOW = 0.01
    top_idx = np.where(losses < best_loss + LOSS_WINDOW)[0]
    
    top_P = np.array(P_runs)[top_idx]
    top_Z = np.array(Z_runs)[top_idx]
    top_r2 = np.array([results_list[i]['r2'] for i in top_idx])
    top_wr2 = np.array([results_list[i]['wr2'] for i in top_idx])

    print(f"\n--- Results Summary ---")
    print(f"Best Loss: {best_loss:.6f}")
    print(f"Mean R^2 (Top Solutions): {np.mean(top_r2):.4f}")
    print(f"Mean Weighted R^2 (Top Solutions): {np.mean(top_wr2):.4f}")
# --- Updated Analysis Section ---
    # 1. Define your Anchor (e.g., the mutant with the most "spread" across conditions)
    # Using the first seed to find a good landmark
    temp_P = np.array(P_runs[0])
    ANCHOR_IDX = np.argmax(np.linalg.norm(temp_P - np.mean(temp_P, axis=0), axis=1))
    
    print(f"Anchoring gauge to Mutant Index: {ANCHOR_IDX}")

    # 2. Apply Gauge Fixing to Top Solutions
    aligned_P = []
    aligned_Z = []

    for i in range(len(top_P)):
        Zf, Pf = gauge_fix_posthoc(top_Z[i], top_P[i], ANCHOR_IDX)
        aligned_P.append(Pf)
        aligned_Z.append(Zf)

    aligned_P = np.array(aligned_P)
    aligned_Z = np.array(aligned_Z)

    # 3. Calculate Consistency
    P_consensus = np.mean(aligned_P, axis=0)
    dispersion = np.mean([np.linalg.norm(p - P_consensus) for p in aligned_P])
    print(f"Gauge-fixed dispersion: {dispersion:.4e}")
    ALT_ANCHOR = (ANCHOR_IDX + 5) % ls_obj.M
    
    # Check the first top solution under both anchors
    _, P_anchor_1 = gauge_fix_posthoc(top_Z[0], top_P[0], ANCHOR_IDX)
    _, P_anchor_2 = gauge_fix_posthoc(top_Z[0], top_P[0], ALT_ANCHOR)
    
    dist1 = calculate_intermutant_distances(P_anchor_1)
    dist2 = calculate_intermutant_distances(P_anchor_2)
    
    dist_diff = np.max(np.abs(dist1 - dist2))
    print(f"Invariance Check (Max distance diff between anchors): {dist_diff:.2e}")
    
    if dist_diff < 1e-6:
        print("SUCCESS: The mutation map geometry is invariant to anchor choice.")
    else:
        print("WARNING: Geometric inconsistency detected.")
# Plotting the result
    for m in range(ls_obj.M):
        pts = aligned_P[:, m, :] # All seeds for mutant m
        plt.scatter(pts[:, 0], pts[:, 1], alpha=0.1, s=5)
    
    plt.scatter(P_consensus[:, 0], P_consensus[:, 1], color='red', marker='x', label='Consensus')
    plt.title("Mouse Mutant Phenotypes (Gauge Fixed)")
