...
MiceG12C=pd.read_csv(r"Documents\McFarlandLabProjects\Figure5A.csv")
MiceG12D=pd.read_csv(r"Documents\McFarlandLabProjects\Figure5B.csv")
MiceEGFR=pd.read_csv(r"Documents\McFarlandLabProjects\Figure5D.csv")
...
#AllMiceVals.to_csv(r"Downloads\BlairMouseData.csv",index=False)
...
def plot_LOO_error_distribution(errors):
    plt.figure(figsize=(7,4))
    sns.histplot(np.array(errors), bins=20, kde=True, color='darkblue')
    plt.axvline(0, color='black', linestyle='--')
    plt.title("LOO Prediction Error Distribution (Predicted - Observed)")
    plt.xlabel("Error")
    plt.ylabel("Frequency")
    plt.tight_layout()
    plt.show()
plot_LOO_error_distribution(errs)
